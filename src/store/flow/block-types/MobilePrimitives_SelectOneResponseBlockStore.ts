...
export const getters: GetterTree<IFlowsState, IRootState> = {
  inflatedChoices: (state, getters, rootState, rootGetters): object => {
    ...
    return Object.keys(currentBlock.config.choices).reduce((memo, choiceKey): {
      [key: string]: IResourceDefinition;
    } => {
...
export const actions: ActionTree<IFlowsState, IRootState> = {
  ...
  async editSelectOneResponseBlockChoice({
    ...
    if (getters.allChoicesHaveContent) {
      ...
      if (exitLabel) {
        commit(
          'flow/block_popExitsByLabel',
          {blockId: activeBlock.uuid, exitLabel},
          {root: true},
        )
      }
    }
  ...
  async createWith(
    ...
    const blankPromptResource = await dispatch(
      'flow/flow_addBlankResourceForEnabledModesAndLangs',
      null,
      {root: true},
    )
    const blankQuestionPromptResource = await dispatch(
      'flow/flow_addBlankResourceForEnabledModesAndLangs',
      null,
      {root: true},
    )
    const blankChoicesPromptResource = await dispatch(
      'flow/flow_addBlankResourceForEnabledModesAndLangs',
      null,
      {root: true},
    )

  ...
  <form class="batch-match-audio-files-prompt alert alert-info form-horizontal">
    ...
    <p v-if="data.isFailure" class="alert alert-danger">
      <strong
        ...
        {{ "flow-builder.failed-finding-matches" | trans }}</strong
      >
      {{ "flow-builder.heres-what-we-know" | trans }}
      <em>{{ data.message || "flow-builder.unknown-error-occurred" }}</em
      >.
    </p>
    ...
    <p v-if="isAudioLibraryEmpty" class="alert alert-warning">
      <strong
        ><i class="glyphicon glyphicon-exclamation-sign"></i>
        {{ "flow-builder.empty-audio-library" | trans }}</strong
      >
      {{
        "flow-builder.we-need-audio-files-previously-uploaded-to-audio-lib-to-match-to-blocks"
          | trans
      }}
        ...
        >
          ...
          <span v-if="pattern.length && !isValid" class="help-block">
            <strong
              ...
              {{ "flow-builder.invalid-pattern" | trans }}</strong
            >
            {{ "flow-builder.at-minimum-we-need-two-placeholders" | trans }} <em>[label]</em>,
            <em>[language]</em>.
          </span>
          ...
          <span v-if="data.isEmpty && data.isComplete" class="help-block">
            <strong
              ><i class="glyphicon glyphicon-exclamation-sign"></i>
              {{ "flow-builder.no-matches-found" | trans }}</strong
            >
          ...
          <div class="checkbox">
            <label>
              ...
              {{ "flow-builder.replace-existing-audio-files-on-blocks" | trans }}
            </label>
          </div>
        </div>
      </div>
    ...
    <template v-if="expanded">
      <h5>
        {{
          "flow-builder.three-components-used-to-create-assignment-rules-and-name-audio-files"
            | trans
        }}
      </h5>
      ...
      <table class="table table-striped table-bordered">
        ...
        <tbody>
          ...
          <tr>
            <td>
              [language]
              {{ "flow-builder.X-abbreviations-set-when-creating-tree" | trans({ lang: "" }) }}
            </td>
            <td>EN</td>
          </tr>
          <tr>
      ...
      <p>
        ...
        {{
          "flow-builder.X-are-required-placeholder-components-for-rule-but-additional-designation-optional"
            | trans({ placeholders: "" })
        }}
        {{
    ...
    <fieldset class="text-right btn-toolbar" :disabled="disabled">
      ...
      <button class="btn btn-primary" :disabled="!isValid" @click.prevent="confirm">

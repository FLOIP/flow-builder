<template>
  <div v-if="block" 
      id="viamo-app" 
      class="viamo-app-container flow-builder-block-editor-container">
    <div class="container-full">
      <div class="main-content-container">
        <section id="tree-app">
          <div id="tree-interaction-designer" class="edit-tree">
            <div class="interaction-designer-contents panel panel-default">
              <div class="hidden-print hidden-xs hidden-sm tree-sidebar-menu-container">
                <nav class="bs-docs-sidebar">
                  <div id="tree-sidebar" class="tree-sidebar-menu">
                    <div class="tree-sidebar-edit-block"
                        :data-block-type="block.type"
                        :data-for-block-id="block.uuid">
                      <slot/>
                    </div>
                  </div>
                </nav>
              </div>
            </div>
          </div>
        </section>
      </div>
    </div>
  </div>
</template>

<script lang="ts">
  import 'dist/css/InteractionDesigner.css'
  import 'dist/css/voto3.css'

  import Vue from 'vue'
  import {Component} from 'vue-property-decorator'

  import {IBlock} from '@floip/flow-runner'

  @Component({
    props: {
      block: {
        type: Object as () => IBlock,
        default: null,
      },
    },
  })
  export class FlowBuilderBlockEditorContainer extends Vue { }
  export default FlowBuilderBlockEditorContainer
</script>
